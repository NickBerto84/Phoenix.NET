...
namespace Phoenix.NET.Common.Network
{
    ...
    public class NetworkWorker
    {
        ...
        /// <summary>
        /// Triggered when an PhoenixError is received.
        /// </summary>
        internal event ErrorHandler OnErrorReceived;
        internal delegate void ErrorHandler(PhoenixError error);

        /// <summary>
        /// Triggered when a user submitted packet is received.
        /// </summary>
        internal event MessageHandler OnClientSubmittedPacketReceived;
        internal delegate void MessageHandler(PhoenixPacket packet);

        /// <summary>
        /// Triggered when an PhoenixMeta packet is received.
        /// </summary>
        internal event MetaHandler OnMetaReceived;
        internal delegate void MetaHandler(PhoenixMeta meta);
        ...
        private void HandleReceivedData(object data)
        {
            try
            {
                if (IsAlive)
                {
                    ...
                    else if (data is PhoenixError)
                        InvokeAsyncOnErrorReceived((PhoenixError)data);
                    else if (data is PhoenixMeta)
                        InvokeAsyncOnMetaReceived((PhoenixMeta)data);
                    else
                    {
                        var packet = (PhoenixPacket)data;
        ...
        private void InvokeAsyncOnErrorReceived(PhoenixError error) => OnErrorReceived?.GetInvocationList().ForEach(e => ((ErrorHandler)e).BeginInvoke(error, EndAsyncEventForErrorHandler, EventArgs.Empty));
        ...
        private void InvokeAsyncOnClientSubmittedPacketReceived(PhoenixPacket packet) => OnClientSubmittedPacketReceived?.GetInvocationList().ForEach(e => ((MessageHandler)e).BeginInvoke(packet, EndAsyncEventForMessageHandler, EventArgs.Empty));
        ...
        private void InvokeAsyncOnMetaReceived(PhoenixMeta meta) => OnMetaReceived?.GetInvocationList().ForEach(e => ((MetaHandler)e).BeginInvoke(meta, EndAsyncEventForMetaHandler, EventArgs.Empty));
