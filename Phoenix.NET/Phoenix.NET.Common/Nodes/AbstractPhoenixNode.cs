...
using Phoenix.NET.Common.Extensions;
...
namespace Phoenix.NET.Common.Nodes
{
    /// <summary>
    /// Abstract implementation of IPhoenixNode, with the addition of events and connection methods.
    /// It also provides a method for testing if the connection is working properly: IsPeerAlive().
    /// </summary>
    public abstract class AbstractPhoenixNode<T> : IPhoenixNode
           where T : PhoenixBaseConfig
    {
        #region Static
        static readonly PhoenixDisposableSubscription _disposedSubscription = new PhoenixDisposableSubscription();
        ...
        /// </summary>
        protected volatile PhoenixConcurrentHashSet<ContentHandler> _broadcastHandlers = new PhoenixConcurrentHashSet<ContentHandler>();

        /// <summary>
        /// Instance of IChannelsSubscriptionsDictionary used to store content handlers subscriptions.
        /// </summary>
        protected volatile IChannelsSubscriptionsDictionary<ContentHandler> _channelsSubscriptions = new PhoenixChannelsSubscriptionsDictionary<ContentHandler>();
        ...
        /// <summary>
        /// The Phoenix configuration for this node.
        /// </summary>
        protected T _config;
        ...
        protected abstract Task<bool> Publish(PhoenixPacket packet);
        #endregion
        ...
        {
            return Publish(new PhoenixMessage(Id, targetChannel, propagateThroughHierarchy)
            {
        ...
        public virtual async Task<IDisposableSubscription> Subscribe(string channel, ContentHandler contentHandler)
        {
            ...
            {
                if (await Publish(new PhoenixSubscribe(Id, channel)))
                    return new PhoenixDisposableSubscription(this, channel, contentHandler);
            }
        ...
        public Task<IDisposableSubscription> SubscribeToBroadcast(ContentHandler contentHandler)
        {
            ...
            {
                return TaskEx.FromResult<IDisposableSubscription>(new PhoenixDisposableSubscription(this, null, contentHandler));
            }
        ...
        public virtual Task<bool> Unsubscribe(string channel, ContentHandler contentHandler, bool keepUnderlyingSubscription = false)
        {
            ...
            {
                contentHandler.BeginInvoke(null, new PhoenixContextHook { Unsubscribing = true }, null, null);
                ...
                {
                    return Publish(new PhoenixUnsubscribe(Id, channel));
                }
            }
        ...
        protected Task<bool> IsPeerAlive()
        {
            try
            {
                return Publish(new PhoenixMeta(Id)
                {
                    ACK = null,
        ...
        protected virtual void OnMetaReceived(PhoenixMeta meta)
        {
        }
        ...
        /// </summary>
        /// <param name="error">The PhoenixError received.</param>
        /// <returns>A log string.</returns>
        protected string GetLogForError(PhoenixError error) => $"[Error];{nameof(error.PublisherId)}: {error.PublisherId};{nameof(error.Exception)}: {error.Exception?.GetFullExceptionMessage()}";

        /// <summary>
        /// Handler for an PhoenixError.
        /// Fires a log event if LogErrorsEnable is true.
        /// </summary>
        /// <param name="error">The PhoenixError received.</param>
        protected virtual void OnError(PhoenixError error)
        {
            if (LogErrorsEnable)
        ...
        /// </summary>
        /// <param name="message">The PhoenixMessage received.</param>
        /// <returns>A log string.</returns>
        protected virtual string GetLogForMessageReceived(PhoenixMessage message) => $"[Message];{nameof(message.TargetChannel)}: {message.TargetChannel};{nameof(message.PublicationDateTime)}: {message.PublicationDateTime};{nameof(message.PropagateThroughHierarchy)}: {message.PropagateThroughHierarchy}";

        /// <summary>
        /// Handler for a packet received from the network.
        /// If the packet is of PhoenixMessage type, invokes the ContentHandlers subscribed to the reception of the message.
        /// Fires a log event if LogMessagesEnable is true and the packet is of PhoenixMessage type.
        /// </summary>
        /// <param name="packet">The packet received.</param>
        protected virtual void OnClientSubmittedPacketReceived(PhoenixPacket packet)
        {
            if (packet is PhoenixMessage)
            {
                ...
                if (handlers != null)
                {
                    var ctxHook = new PhoenixContextHook()
                    {
                ...
                if (handlers == null)
                    OnLog?.BeginInvoke(GetLogForError(new PhoenixError(Id)
                    {
