...
namespace Phoenix.NET.Server.PubSub
{
    ...
    /// </summary>
    public class PhoenixPubSubRouter : IPubSubRouter
    {
        ...
        /// Constructor.
        /// </summary>
        /// <param name="subsDictionary">An implementation of IChannelsSubscriptionsDictionary. If not specified, it will use an instance of PhoenixChannelsSubscriptionsDictionary.</param>
        public PhoenixPubSubRouter(IChannelsSubscriptionsDictionary<IMessageSubscriber> subsDictionary = null)
        {
            _subsDictionary = subsDictionary ?? new PhoenixChannelsSubscriptionsDictionary<IMessageSubscriber>();
        }
        ...
        /// <returns>True if the operation succeeded.</returns>
        public bool SubmitMessage(IMessageSubscriber sender, PhoenixMessage message)
        {
            if (message.PublisherId == null || !_nodes.ContainsKey(sender))
                return false;

            PhoenixConcurrentHashSet<Guid> _deliveryNodes = new PhoenixConcurrentHashSet<Guid>();
            ...
            {
                var phoenixNodes = _subsDictionary.GetSubscriptions(message.TargetChannel, message.PropagateThroughHierarchy);
                if (phoenixNodes != null)
                    phoenixNodes.ForEach(sendMessageToOthersAction);

                return phoenixNodes != null;
            }
        }
